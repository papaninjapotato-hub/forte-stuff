...
function setSelection(id) {
  if (selectedPlayer && selectedPlayer !== id) nodes.update({id: selectedPlayer, borderWidth: 6});
  selectedPlayer = id;
  if (id) nodes.update({id, borderWidth: 14});
}
function swapPlayers(aId, bId) {
  ...
  nodes.update([
    {id: aId, x: pos[bId].x, y: pos[bId].y, borderWidth: 6},
    {id: bId, x: pos[aId].x, y: pos[aId].y, borderWidth: 6}
  ]);
