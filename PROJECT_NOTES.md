...
## Files
- `build_graph.py` — single script that parses the CSV and emits `graph.html` (beta). Does NOT overwrite `index.html`.
...
- `index.html` — stable/production page. Manually maintained. Has a "🧪 Beta" link to `graph.html`.
- `graph.html` — beta page generated by `build_graph.py`. Has a "← Stable" link back to `index.html`.
